...
const BillingItem = ({ user }: { user: UserPayload | null }) => {
  ...
  return (
    ...
    >
      <BillingIcon className={cn('w-5 h-5 mr-3', isActive && 'text-primary')} />
      {billingLabel}
    </Link>
  );
};
...
export function Sidebar() {
  ...
  // Effet pour gérer l'ouverture/fermeture automatique des menus
  useEffect(() => {
    // Fermer tous les menus d'abord
    const newOpenMenus: Record<string, boolean> = {};
    
    // Trouver et ouvrir le menu actif
    for (const item of getNavItems()) {
      if (item.subItems) {
        ...
        if (hasActiveChild) {
          ...
          break; // On ne garde qu'un seul menu ouvert à la fois
        }
      }
    }
